...
# ret, thresh = cv2.threshold(gray, 107, 255,  cv2.THRESH_BINARY_INV)
...
def video_stream():
    ...
    imrgb = imrgb[:350, 100:frame.shape[1] - 160]
    imrgb = cv2.resize(imrgb, (400, 400))
    imgray = cv2.cvtColor(imrgb, cv2.COLOR_BGR2GRAY)

    # ret, imthresh = cv2.threshold(imgray, 70, 250, cv2.THRESH_BINARY_INV)

    img = cv2.blur(imrgb, (5, 5))

    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)

    # imthresh = cv2.adaptiveThreshold(v, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)

    imthresh = cv2.adaptiveThreshold(v, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 2)
...
# def drawX(start, end):
#     cv2.line(grays, (start[0], start[1]), (end[0], end[1]), (0, 0, 0), 3)
#     cv2.line(grays, (start[0], end[1]), (end[0], start[1]), (0, 0, 0), 3)
...
def process():
    ...
    thresh2 = cv2.adaptiveThreshold(v, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 2)
    ...
    find = find_image(gray, thresh2, contours)
    data = find.listData
